...
# Workaround: .config file conflicts with Prisma config resolution
if [ -f "$PROJECT_DIR/.config" ]; then
  mv "$PROJECT_DIR/.config" "$PROJECT_DIR/.config.juicefs"
  bun run db:push
  mv "$PROJECT_DIR/.config.juicefs" "$PROJECT_DIR/.config"
else
  bun run db:push
fi
